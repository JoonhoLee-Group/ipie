...
def get_local_e(rh1, Ghalf, rchol, nuc):
    ...
    '''
    local_e1 = 2 * (torch.einsum('ij, aji->a', rh1, Ghalf.real) + 1j * torch.einsum('ij, aji->a', rh1, Ghalf.imag))
    local_e2 = .5 * (get_Coulomb(rchol, Ghalf) - get_Exchange(rchol, Ghalf))
    local_nuc = nuc
...
def get_trial_e(rh1, G, rchol, nuc):
    ...
    local_e2 = .5 * (get_Coulomb_trial(rchol, G) - get_Exchange_trial(rchol, G))
    local_nuc = nuc
    local_e = local_nuc + local_e1 + local_e2
    return local_e
...
def get_Coulomb(rchol, Ghalf):
    ...
    '''
    X = 2 * (torch.einsum('pij, aji -> ap', rchol, Ghalf.real) + 1j * torch.einsum('pij, aji->ap', rchol, Ghalf.imag))
...
def get_Exchange(rchol, Ghalf):
    ...
    input:
        ...
        ghalf, 3d tensor with size (nwalkers, nocc, nao), the green's function of the trial wavefunction
    '''
    EXreal = 2 * (torch.einsum('gip, apj, gjq, aqi -> a', rchol, Ghalf.real, rchol, Ghalf.real) - torch.einsum('gip, apj, gjq, aqi -> a', rchol, Ghalf.imag, rchol, Ghalf.imag))
    EXimag = 1j * 2 * (torch.einsum('gip, apj, gjq, aqi -> a', rchol, Ghalf.real, rchol, Ghalf.imag) + torch.einsum('gip, apj, gjq, aqi -> a', rchol, Ghalf.imag, rchol, Ghalf.real))
    EX = EXreal + EXimag
    return EX
