...
class SDTrial:
    ...
    def calc_overlap(self, walker_states):
        return torch.einsum('ij,aik->ajk', self.psi.conj(), walker_states.real) + 1j * torch.einsum('ij,aik->ajk', self.psi.conj(), walker_states.imag)
    ...
    def calc_force_bias(self, walkers):
        ...
        vbias= 2.0 * (torch.einsum('pij,aji->ap', self.rchol, Ghalf.real) + 1j * torch.einsum('pij,aji->ap', self.rchol, Ghalf.imag))
        return vbias
