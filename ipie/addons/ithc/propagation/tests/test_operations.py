...
from ipie.addons.ithc.propagation.operations import (
    apply_exponential,
...
def test_apply_isometry():
    ...
    isometry = numpy.random.randn(nbasis**2).reshape(nbasis, nbasis)

    phi_ref = numpy.einsum("mn,wni->wmi", isometry, phi)
    phi = apply_isometry(phi, isometry)
