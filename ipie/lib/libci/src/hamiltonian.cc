...
Hamiltonian::Hamiltonian(
    ...
    h1e_c.resize(num_spatial * num_spatial);
    for (size_t p = 0; p < num_spatial; p++) {
        for (size_t q = 0; q < num_spatial; q++) {
            for (int r = 0; r < num_spatial; r++) {
                h1e_c[flat_indx(p, q)] += h2e[flat_indx(p, r, r, q)];
            }
        }
    }
}
...
ipie::complex_t Hamiltonian::get_h1e(size_t p, size_t q) const {
    return h1e[flat_indx(p, q)];
}

ipie::complex_t Hamiltonian::get_h1e_c(size_t p, size_t q) const {
    return h1e_c[flat_indx(p, q)];
}

ipie::complex_t Hamiltonian::get_h2e(size_t p, size_t q, size_t r, size_t s) const {
    return h2e[flat_indx(p, q, r, s)];
}
...
ipie::energy_t slater_condon1(const Hamiltonian &ham, const std::vector<size_t> &occs, const Excitation &excit_ia) {
    ...
    //         size_t iajj =
    //             ham.flat_indx(i_spat_spin.first, a_spat_spin.first, occ_j_spat_spin.first,
    //             occ_j_spat_spin.first);
    //         hmatel.e2b += ham.h2e[iajj];
    //         if (occ_j_spat_spin.second == i_spat_spin.second) {
    //             size_t ijja =
    //                 ham.flat_indx(i_spat_spin.first, occ_j_spat_spin.first, occ_j_spat_spin.first,
    //                 a_spat_spin.first);
