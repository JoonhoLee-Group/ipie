...
def test_hubbard():
    ...
    assert local_energy(system, ham, walker1, trial)[0] == pytest.approx(
        local_energy(system, ham, walker2, trial)[0]
    )
...
def test_propagate_walker_free():
    ...
    numpy.random.seed(7)
    qmc = dotdict({"dt": dt, "nstblz": 1})
    prop = ThermalDiscrete(
        ham,
        trial,
        qmc,
        {"charge_decomposition": False, "free_projection": True},
        verbose=False,
    )
