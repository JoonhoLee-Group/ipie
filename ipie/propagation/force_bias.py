...
def construct_force_bias_batch(hamiltonian, walker_batch, trial, mpi_handler=None):
    ...
    if (walker_batch.name == "SingleDetWalkerBatch" and trial.name == "MultiSlater"):
        if (hamiltonian.chunked):
            return construct_force_bias_batch_single_det_chunked(hamiltonian, walker_batch, trial, mpi_handler)
        else:
...
def construct_force_bias_batch_single_det(hamiltonian, walker_batch, trial):
    ...
    if (walker_batch.rhf):
        ...
        if trial.mixed_precision:
            dGhalfa = walker_batch.Ghalfa.reshape(walker_batch.nwalkers, walker_batch.nup*hamiltonian.nbasis) - trial.psia.T.ravel()
            dGhalfb = walker_batch.Ghalfb.reshape(walker_batch.nwalkers, walker_batch.ndown*hamiltonian.nbasis) - trial.psib.T.ravel()
            dGhalfa = dGhalfa.astype(numpy.complex64)
            dGhalfb = dGhalfb.astype(numpy.complex64)
            if isrealobj(trial._rchola) and isrealobj(trial._rcholb):
                # single precision
                vbias_batch_real = trial._rchola.dot(dGhalfa.T.real) + trial._rcholb.dot(dGhalfb.T.real)
                vbias_batch_imag = trial._rchola.dot(dGhalfa.T.imag) + trial._rcholb.dot(dGhalfb.T.imag)
                # double precision
...
def construct_force_bias_batch_single_det_chunked(hamiltonian, walker_batch, trial, handler):
    """Compute optimal force bias.
    ...
    vbias_batch_real_send[chol_idxs_chunk,:] = trial._rchola_chunk.dot(Ghalfa.T.real) + trial._rcholb_chunk.dot(Ghalfb.T.real)
    vbias_batch_imag_send[chol_idxs_chunk,:] = trial._rchola_chunk.dot(Ghalfa.T.imag) + trial._rcholb_chunk.dot(Ghalfb.T.imag)
    ...
    for icycle in range(handler.ssize-1):
        ...
        vbias_batch_real_send[chol_idxs_chunk,:] = trial._rchola_chunk.dot(Ghalfa_recv.T.real) + trial._rcholb_chunk.dot(Ghalfb_recv.T.real)
        vbias_batch_imag_send[chol_idxs_chunk,:] = trial._rchola_chunk.dot(Ghalfa_recv.T.imag) + trial._rcholb_chunk.dot(Ghalfb_recv.T.imag)
