...
def calc_noper_extended(psi0, psi0_extended, phi_batch, phi_extended_batch):
    ...
    if is_cupy(psi0):
        ...
        n_batch = xp.einsum("ni,wij,wnj->wn", psi0_extended.conj(), inv_O, phi_extended_batch, optimize=True)
...
class PhaselessITHC(PhaselessBase):
    ...
    def build(self, hamiltonian:GenericITHC ,trial=None, walkers=None, mpi_handler=None, verbose=False, mean_field_shift = True):
        ...
        self.expH1 = xp.array([scipy.linalg.expm(-0.5 * self.dt * H1_numpy[0]), scipy.linalg.expm(-0.5 * self.dt * H1_numpy[1])])
    ...
    def propagate_walkers_two_body_first_order(self, walkers, hamiltonian:GenericITHC, trial, half_rotate= True): #applies discrete HS transform and updates weights consider spin
        # force bias
