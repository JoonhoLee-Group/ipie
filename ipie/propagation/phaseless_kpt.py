...
class PhaselessKptChol(PhaselessKptBase):
    ...
    def construct_VHS(self, hamiltonian: KptComplexChol, xshifted: xp.ndarray) -> xp.ndarray:
        ...
        VHS = numpy.zeros(
            (nwalkers, hamiltonian.nk, hamiltonian.nbasis, hamiltonian.nk, hamiltonian.nbasis),
        ...
        for iq in range(hamiltonian.nk):
            for ik in range(hamiltonian.nk):
                ...
                VHS[:, ik, :, ikpq, :] = self.sqrt_dt * numpy.einsum(
                    "wx, xpr -> wpr", xvhsiq, hamiltonian.chol[:, ik, :, iq, :]
                )
        VHS = VHS.reshape(
            nwalkers, hamiltonian.nk * hamiltonian.nbasis, hamiltonian.nk * hamiltonian.nbasis
        )
...
class PhaselessKptCholChunked(PhaselessKptChol):
    ...
    @plum.dispatch
    def construct_VHS(self, hamiltonian: KptComplexCholChunked, xshifted: xp.ndarray) -> xp.ndarray:
        ...
        chol_chunk = hamiltonian.chol_chunk.reshape(
            -1, hamiltonian.nk, hamiltonian.nbasis, hamiltonian.unique_nk, hamiltonian.nbasis
        )
        if config.get_option("use_gpu"):
            if hamiltonian.nk > 256:
                # nk^2 > 65536, use cuquantum instead
                VHS_send = construct_VHS_cuquantum(
                    chol_chunk,
                    self.sqrt_dt,
                    xshifted[:, :, idxs, :],
                    hamiltonian.nk,
                ...
                VHS_send = construct_VHS_symm_gpu(
                    chol_chunk,
                    self.sqrt_dt,
                    xshifted[:, :, idxs, :],
                    hamiltonian.nk,
            ...
            VHS_send = construct_VHS_kernel_symm(
                chol_chunk,
                self.sqrt_dt,
                xshifted[:, :, idxs, :],
                hamiltonian.nk,
...
Phaseless = {
    "cholesky": PhaselessKptChol,
    "isdf": PhaselessKptISDF,
    "cholchunked": PhaselessKptCholChunked,
}
