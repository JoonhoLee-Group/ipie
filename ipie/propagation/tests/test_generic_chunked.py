...
from ipie.propagation.force_bias import construct_force_bias_batch_single_det, construct_force_bias_batch_single_det_chunked
...
def test_generic_propagation_chunked():
    ...
    walker_batch = SingleDetWalkerBatch(system, ham, trial, nwalkers, mpi_handler=mpi_handler)
    ...
    vfb_chunked = construct_force_bias_batch_single_det_chunked(ham, walker_batch, trial, mpi_handler)

    assert(numpy.allclose(vfb, vfb_chunked))
    xshifted = numpy.random.normal(0.0, 1.0, ham.nchol*walker_batch.nwalkers).reshape(walker_batch.nwalkers, ham.nchol)
    VHS_chunked = prop.propagator.construct_VHS_batch_chunked(ham, xshifted.T.copy(), walker_batch.mpi_handler)
