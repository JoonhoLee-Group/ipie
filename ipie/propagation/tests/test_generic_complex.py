...
def test_vhs_complex():
    ...
    xshifted = numpy.random.normal(
        0.0, 1.0, nwalkers * ham.nfields
    ).reshape(ham.nfields,nwalkers)

    vhs = test_handler.propagator.construct_VHS(
        ham, xshifted
    )

    isqrt_dt = 1.j * numpy.sqrt(qmc.dt)
...
def test_vhs_complex_vs_real():
    ...
    cx_ham = GenericComplexChol(numpy.array(ham.H1,dtype=numpy.complex128), cx_chol, ham.ecore, verbose=False)
    nchol = cx_chol.shape[-1]
    cx_xshifted = numpy.zeros((cx_ham.nfields, nwalkers))
    cx_xshifted[:nchol,:] = xshifted.copy()
    cx_xshifted[nchol:,:] = numpy.random.normal(
        0.0, 1.0, nwalkers * ham.nfields
    ).reshape(nchol,nwalkers)
    cx_vhs = test_handler.propagator.construct_VHS(
        cx_ham, cx_xshifted
    )
