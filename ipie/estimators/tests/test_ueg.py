...
from ipie.estimators.ueg import fock_ueg, local_energy_ueg
...
def test_build_J():
    ...
    from ipie.estimators.ueg import coulomb_greens_function
    for s in [0,1]:
        coulomb_greens_function(len(ham.qvecs), ham.ikpq_i,
                ham.ikpq_kpq, ham.ipmq_i, ham.ipmq_pmq, Gkpq[s],
                Gpmq[s], G[s])

    from ipie.estimators.ueg import build_J
    J1 = timeit(build_J)(ham, Gpmq, Gkpq)
    from ipie.estimators.ueg_kernels import build_J_opt
...
def test_build_K():
    ...
    G = numpy.array([gab(psi[:,:sys.nup], psi[:,:sys.nup]),
                     gab(psi[:,sys.nup:],
                         psi[:,sys.nup:])]).astype(numpy.complex128)
    from ipie.estimators.ueg import build_K
    from ipie.estimators.ueg_kernels import build_K_opt
