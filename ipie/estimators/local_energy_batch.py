...
def local_energy_batch(hamiltonian, walkers, trial):
    ...
    if isinstance(walkers, UHFWalkers):
        if config.get_option("use_gpu"):
            if hamiltonian.chunked:
                ...
                return local_energy_single_det_batch_gpu(hamiltonian, walkers, trial)
        elif walkers.rhf:
            return local_energy_single_det_rhf_batch(hamiltonian, walkers, trial)
        else:
            if hamiltonian.chunked:
...
def local_energy_multi_det_trial_batch(hamiltonian, walkers, trial):
    ...
    for _, (w, Ga, Gb, Ghalfa, Ghalfb) in enumerate(
        ...
        for idet in range(ndets):
            ...
            e = list(local_energy_G(hamiltonian, trial, G, Ghalf=None))
            numer0 += w[idet] * e[0]
            numer1 += w[idet] * e[1]
            numer2 += w[idet] * e[2]
            denom += w[idet]
