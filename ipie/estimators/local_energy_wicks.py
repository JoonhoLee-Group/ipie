    ...
    for iw in range(nwalkers):
        ...
        for x in range(nchol):
            ...
            Ttilde = (
                numpy.dot(
                    theta_act_real_a.T,
                    rchol_a[x].reshape((nocc_a, nbasis))
                    )
                +
                1j*numpy.dot(
                    theta_act_imag_a.T,
            ...
            Q = rchol_act_a[x].reshape((nact, nbasis)) - Ttilde
            A = numpy.dot(Q.T, CI_a[iw]) # Q_tr I_ta -> A_{ra}
            B = numpy.dot(theta_a[iw], A) # theta_{br} A_{ra} -> C_{ba}
            F[x] += numpy.trace(B[nfrozen_a:nfrozen_a+nocc_act_a,:])
            # index a gets contracted with CI tensor later so only need the
            # occupied orbitals in the active space not the full set of occupied.
            cont2_K[iw] -= numpy.dot(T[nfrozen_a:nfrozen_a+nocc_act_a,:].ravel(), B.T.ravel())
            Lvo[0,iw,x,:,:] = numpy.dot(Q, theta_occ_a[nfrozen_a:nfrozen_a+nocc_act_a].T)
            # beta contributions
            T = (
                    numpy.dot(
                        theta_occ_real_b,
                        rchol_b[x].reshape((nocc_b, nbasis)).T
                        )
                    +
                    1j*numpy.dot(
                        theta_occ_imag_b,
            ...
            Ttilde = (
                numpy.dot(
                    theta_act_real_b.T,
                    rchol_b[x].reshape((nocc_b, nbasis))
                    )
                +
                1j*numpy.dot(
                    theta_act_imag_b.T,
            ...
            Q = rchol_act_b[x].reshape((nact, nbasis)) - Ttilde
            A = numpy.dot(Q.T, CI_b[iw]) # Q_tr I_ta -> A_{ra}
            B = numpy.dot(theta_b[iw], A) # theta_{br} A_{ra} -> C_{ba}
            F[x] += numpy.trace(B[nfrozen_b:nfrozen_b+nocc_act_b,:])
            # index a gets contracted with CI tensor later so only need the
            # occupied orbitals in the active space not the full set of occupied.
            cont2_K[iw] -= numpy.dot(T[nfrozen_b:nfrozen_b+nocc_act_b,:].ravel(), B.T.ravel())
            Lvo[1,iw,x,:,:] = numpy.dot(Q, theta_occ_b[nfrozen_b:nfrozen_b+nocc_act_b].T)
        cont1_J[iw] += 0.5*numpy.dot(X, X)
        cont2_J[iw] += numpy.dot(F, X)

    return cont1_J+cont1_K, cont2_J+cont2_K

def local_energy_multi_det_trial_wicks_batch_opt_old(system, ham, walker_batch, trial):
    nwalkers = walker_batch.nwalkers
    nbasis = ham.nbasis
    nchol = ham.nchol
    nalpha = system.nup
    ...
    Laa = numpy.zeros((nwalkers, nchol, trial.nact, trial.nocc_alpha), dtype=numpy.complex128)
    Lbb = numpy.zeros((nwalkers, nchol, trial.nact, trial.nocc_beta), dtype=numpy.complex128)
    cont2_Kaa = build_exchange_contributions_opt(
                                    trial._rchola,
                                    trial._rchola_act,
                                    walker_batch.Ghalfa,
                                    walker_batch.CIa,
    ...
    cont2_Kbb = build_exchange_contributions_opt(
                                    trial._rcholb,
                                    trial._rcholb_act,
                                    walker_batch.Ghalfb,
                                    walker_batch.CIb,
    ...
    Laa = Laa.transpose((0, 2, 3, 1)).copy()
    Lbb = Lbb.transpose((0, 2, 3, 1)).copy()
