...
def test_from_chkfile():
    ...
    mf.chkfile = "scf.chk"
    mf.kernel()
    h1e, chol, enuc, basis_change_mat = integrals_from_chkfile("scf.chk", verbose=0, chol_cut=1e-5)
...
def test_pyscf_to_ipie():
    ...
    mf.chkfile = "scf.chk"
    mf.kernel()
    gen_ipie_input_from_pyscf_chk("scf.chk", hamil_file="afqmc.h5", verbose=False)
    ...
    gen_ipie_input_from_pyscf_chk(
        "scf.chk", hamil_file="afqmc.h5", num_frozen_core=2, verbose=False
    )
    wfn = read_wavefunction("wavefunction.h5")
    ...
    coeff, occa, occb = zip(*fci.addons.large_ci(fcivec, 2, (1, 1), tol=1e-8, return_strs=False))
    with h5py.File("scf.chk", "r+") as fh5:
        fh5["mcscf/ci_coeffs"] = coeff
        fh5["mcscf/occs_alpha"] = occa
        fh5["mcscf/occs_beta"] = occb
    gen_ipie_input_from_pyscf_chk(
        "scf.chk", hamil_file="afqmc.h5", num_frozen_core=2, verbose=False, mcscf=True
    )
    wfn = read_wavefunction("wavefunction.h5")
    assert len(wfn[0]) == 3
...
def test_frozen_uhf():
    ...
    gen_ipie_input_from_pyscf_chk(
        "scf.chk", hamil_file="afqmc.h5", verbose=False, num_frozen_core=ncore
    )
    wfn = read_wavefunction("wavefunction.h5")
