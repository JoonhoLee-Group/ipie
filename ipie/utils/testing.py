...
from ipie.hamiltonians.generic_ithc import eri_diag
...
def gen_random_test_instances_ithc(nmo, nocc, naux, nwalkers, seed=7, ndets=1):
    ...
    numpy.random.seed(seed)
    wfn = get_random_nomsd(nocc, nocc, nmo, ndet=ndets)
    h1e, chol, nuc, eri = generate_hamiltonian(nmo, nocc, cplx=True, sym=4)
    isometry = 0.005 * (
        numpy.random.randn(nmo,naux)
        + 1.0j * numpy.random.randn(nmo,naux)
    )
