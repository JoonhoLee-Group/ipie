...
def read_qmcpack_hamiltonian(filename, get_chol=True):
    ...
    try:
        ...
        pass
    try:
        hc, chol, enuc, nmo, nelec = read_qmcpack_sparse(filename, get_chol=get_chol)
        hamil = {"hcore": hc, "chol": chol, "enuc": enuc, "nmo": nmo, "nelec": nelec}
        return hamil
    except KeyError:
        pass
    try:
                            ...
                            for l in range(0, nmo_pk[kl]):
                                ...
                                for j in range(0, nmo_pk[kj]):
                                    ...
                                    if abs(eri[ik, lj]) > tol and sym_allowed:
                                        out = fmt_integral(
                                            eri[ik, lj], I, K, J, L, cplx, paren=paren
                                        )
                                        f.write(out)
                                    lj += 1
                            ik += 1
            ...
            for i in range(nmo_pk[ik]):
                ...
                for j in range(nmo_pk[ik]):
                    ...
                    if I >= J and abs(hk[i, j]) > tol:
                        out = fmt_integral(hk[i, j], I, J, -1, -1, cplx, paren=paren)
                        f.write(out)
