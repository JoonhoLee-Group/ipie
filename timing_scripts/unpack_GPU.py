...
end_time = time.time()
print(f"original: {end_time - start_time}")
...
print(f"packed gemm (CPU): {tpacked_gemm}")
...
print(f"unpacking (CPU): {tunpacking}")
print(f"packed total: {tpacked_gemm + tunpacking}")
...
end_time = time.time()
tpacked_gemm = end_time - start_time
print(f"packed gemm (GPU, DP): {tpacked_gemm}")
...
unpack_VHS_batch_gpu[blockspergrid, threadsperblock](idx_i, idx_j, VHS_packed, VHS_unpacked)
end_time = time.time()
print(f"unpacking (GPU): {end_time - start_time}")
