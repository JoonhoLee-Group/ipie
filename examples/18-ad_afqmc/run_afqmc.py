...
from ipie.addons.adafqmc.utils.miscellaneous import (
...
mo_dip_act = torch.zeros(
    (ao_dip.shape[0], mol.nao - num_frozen, mol.nao - num_frozen), dtype=torch.float64
)
for i in range(ao_dip.shape[0]):
    mo_dip_act[i] = mo_dip[i][num_frozen:, num_frozen:]
    nuc_dip[i] += 2.0 * torch.trace(mo_dip[i][:num_frozen, :num_frozen])

dip = (mo_dip_act[1], torch.tensor([nuc_dip[1]]))

hamobs = generate_hamiltonian_from_pyscf(
    mf, num_frozen=num_frozen, ortho_ao=False, observable=dip, obs_type="dipole"
)
...
orthtrial, _ = torch.linalg.qr(
    basis_change_matrix[:, num_frozen:].t() @ overlap @ basis_change_matrix[:, num_frozen:]
)

coupling = torch.tensor([0.0], dtype=torch.float64, requires_grad=True)

partial_trial = partial(
    get_hf_wgradient,
    orthtrial,
    mol.nelec[0] - num_frozen,
    torch.eye(mol.nao_nr() - num_frozen, dtype=torch.float64),
    hamobs,
    "RHF",
)
trial_nondetached, grad = jvp(
    partial_trial, (coupling,), (torch.tensor([1.0], dtype=torch.float64),)
)
