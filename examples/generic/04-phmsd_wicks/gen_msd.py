...
h1e, chol, nelec, cas_idx = generate_integrals(
    mol, mf.get_hcore(), mo, chol_cut=1e-5, verbose=True
)
write_qmcpack_dense(
    h1e, chol.T.copy(), nelec, h1e.shape[-1], enuc=enuc, filename="afqmc_msd.h5"
)
coeff, occa, occb = zip(
    *fci.addons.large_ci(fcivec, M, (nocca, noccb), tol=1e-14, return_strs=False)
)
...
write_qmcpack_wfn(
    "afqmc_msd.h5", (coeff, occa, occb), "uhf", mol.nelec, nmo, init=psi0, mode="a"
)
