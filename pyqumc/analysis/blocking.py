#!/usr/bin/env python
'''Run a reblocking analysis on pyqumc QMC output files.'''
...
from pyqumc.utils.misc import get_from_dict
from pyqumc.utils.linalg import get_ortho_ao_mod

from pyqumc.analysis.autocorr import reblock_by_autocorr
...
def analyse_simple(files, start_time):
    data = pyqumc.analysis.extraction.extract_hdf5_data_sets(files)
    norm_data = []
    for (g, f) in zip(data, files):
        ...
        if free_projection:
            ...
            columns = pyqumc.analysis.extraction.set_info(reblocked, m)
