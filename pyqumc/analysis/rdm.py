import pandas as pd
import numpy
from pyqumc.analysis.extraction import extract_rdm, get_param
...
def analyse_one_body(filename, one_body, est_type='back_propagated',
                     rdm_type='one_rdm', skip=1):
    ...
    es, errs = zip(*res)
    data = pd.DataFrame({'tau': numpy.array(splits)*dt,
                         'OneBody': es,
                         'OneBody_error': errs})
    return data
