...
Here we'll outline how use pie to calculate the imaginary time correlation function
...
First we'll run pie as follows:

.. code-block:: bash

    $ python3 -u ~/path/to/pie/bin/pie input.json > 3x3.out

By default pie will print some basic information to stdout, including periodic estimates
...
.. code-block:: bash
    ...
    $ ~/path/to/pie/tools/reblock.py -s 10 -f estimates.0.h5
...
.. code-block:: bash
    ...
    $ ~/path/to/pie/tools/extract_observable.py -o 'energy' > basic.out
...
.. code-block:: bash
    ...
    $ ~/path/to/pie/tools/extract_observable.py -o 'back_propagated' > back_propagated.out
...
.. code-block:: bash
    ...
    $ ~/path/to/pie/tools/extract_observable.py -o 'itcf' -s up -t greater -e 0,0 -f analysed_observables.h5 > itcf.out
