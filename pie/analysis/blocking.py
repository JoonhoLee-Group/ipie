#!/usr/bin/env python
'''Run a reblocking analysis on pie QMC output files.'''
...
from pie.utils.misc import get_from_dict
from pie.utils.linalg import get_ortho_ao_mod

from pie.analysis.autocorr import reblock_by_autocorr
...
def analyse_simple(files, start_time):
    data = pie.analysis.extraction.extract_hdf5_data_sets(files)
    norm_data = []
    for (g, f) in zip(data, files):
        ...
        if free_projection:
            ...
            columns = pie.analysis.extraction.set_info(reblocked, m)
