import numpy
try:
    from pie.estimators.ueg import local_energy_ueg
    from pie.estimators.pw_fft import local_energy_pw_fft
except ImportError as e:
    print(e)
from pie.estimators.hubbard import local_energy_hubbard, local_energy_hubbard_ghf,\
...
from pie.estimators.ci import get_hmatel
from pie.estimators.thermal import particle_number, one_rdm_from_G
        ...
        if Ghalf is not None:
            if hamiltonian.stochastic_ri and hamiltonian.control_variate:
                return local_energy_generic_cholesky_opt_stochastic(system, G,
                                     ...
                                     Ghalf=Ghalf,
                                     rchol=trial._rchol, C0=trial.psi, ecoul0=trial.ecoul0,
                                     exxa0=trial.exxa0,
                                     exxb0=trial.exxb0)
...
def local_energy(system, hamiltonian, walker, trial):
    if walker.name == "MultiDetWalker":
        if hamiltonian.name == "HubbardHolstein":
            return local_energy_multi_det_hh(system, walker.Gi, walker.weights, walker.X, walker.Lapi)
        else:
        ...
        if hamiltonian.name == "HubbardHolstein":
            return local_energy_G(system, hamiltonian, trial, walker.G, walker.Ghalf, walker.X, walker.Lap)
        else:
...
def variational_energy_ortho_det(system, ham, occs, coeffs):
    ...
    Parameters
    ----------
    system : :class:`pie.system` object
        System object.
    occs : list of lists
