...
from pie.hamiltonians.ueg import UEG as HamUEG
from pie.estimators.ueg import fock_ueg, local_energy_ueg
from pie.estimators.greens_function import gab
from pie.utils.testing import get_random_wavefunction
from pie.utils.misc import timeit
...
def test_build_J():
    ...
    from pie.estimators.ueg import coulomb_greens_function
    for s in [0,1]:
        coulomb_greens_function(len(ham.qvecs), ham.ikpq_i,
                ham.ikpq_kpq, ham.ipmq_i, ham.ipmq_pmq, Gkpq[s],
                Gpmq[s], G[s])

    from pie.estimators.ueg import build_J
    J1 = timeit(build_J)(ham, Gpmq, Gkpq)
    from pie.estimators.ueg_kernels import build_J_opt
    J2 = timeit(build_J_opt)(len(ham.qvecs), ham.vqvec, ham.vol, ham.nbasis,
...
def test_build_K():
    ...
    G = numpy.array([gab(psi[:,:sys.nup], psi[:,:sys.nup]),
                     gab(psi[:,sys.nup:],
                         psi[:,sys.nup:])]).astype(numpy.complex128)
    from pie.estimators.ueg import build_K
    from pie.estimators.ueg_kernels import build_K_opt
    K1 = timeit(build_K)(ham, G)
    K2 = timeit(build_K_opt)(len(ham.qvecs), ham.vqvec, ham.vol, ham.nbasis,
                        ham.ikpq_i, ham.ikpq_kpq, ham.ipmq_i, ham.ipmq_pmq,
                        G)
